...
class DQN:
    ...
    def save(self, model_path, checkpoint=False):
        if checkpoint:
            ...
            torch.save({'behavior_net': self._behavior_net.state_dict(),}, model_path)
    ...
    for episode in range(args.episode):
        ...
        for t in itertools.count(start=1):
            ...
            if done:
                ...
                break
    env.close()
    return steps, ewma_reward_list, total_reward_list
...
def main():
    ...
    parser.add_argument('-m', '--model', default='./models/ddqn.pth')
    ...
    if not args.test_only:
        steps, ewma_reward_list, total_reward_list = train(args, env, agent, writer)
        plot_curve('DQN', steps, ewma_reward_list, total_reward_list)
