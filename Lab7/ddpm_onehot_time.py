...
import numpy as np
from model import UNet_conditional_onehot_time
...
def plot(eval_acc_list, avg_loss_list, args):
    _e = np.linspace(1, args.epochs, args.epochs)

    plt.figure()
    plt.title('Loss G/Loss D')
    plt.plot(_e, avg_loss_list, color='royalblue', label='Generator loss')
...
class Diffusion:
    ...
    def one_hot(self, a, num_classes):
        return torch.from_numpy(np.squeeze(np.eye(num_classes)[a.reshape(-1)]))

    def sample_timesteps(self, n):
        rand = torch.randint(low=0, high=self.noise_steps - 1, size=(n,))
        return self.one_hot(rand, self.noise_steps).to(device), rand.to(device)
    ...
    def sample(self, model, n, conds, num, args, invTrans):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(reversed(range(1, self.noise_steps)), position=0):
                ...
                t_onehot = self.one_hot(t, args.noise_steps).to(self.device)
                predicted_noise = model(x, t_onehot, conds)
                if args.cfg_scale > 0:
                    uncond_predicted_noise = model(x, t_onehot, None)
...
def train(model, diffusion, optimizer, train_loader, test_loader, loss_func, writer, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, conds) in enumerate(tqdm(train_loader)):
            ...
            t_onehot, t = diffusion.sample_timesteps(images.shape[0])
            x_t, noise = diffusion.noise_images(images, t)


            if np.random.random() < 0.1:
                conds = None
            noise_pred = model(x_t, t_onehot, conds)
    ...
    plot(eval_acc_list, avg_loss_list, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', default='log/ddpm/B')
    ...
    parser.add_argument('--exp_name', type=str, default='B')
    ...
    model = UNet_conditional_onehot_time(time_dim=args.time_dim, num_conditions=args.num_conditions, noise_steps=args.noise_steps).to(device)
